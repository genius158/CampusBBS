...
public class PtrFrameLayout extends in.srain.cube.views.ptr.PtrFrameLayout {
    ...
    // config
    private in.srain.cube.views.ptr.PtrFrameLayout.Mode mMode = in.srain.cube.views.ptr.PtrFrameLayout.Mode.BOTH;
    ...
    private in.srain.cube.views.ptr.PtrFrameLayout.Mode getModeFromIndex(int index) {
        switch (index) {
            case 0:
                return in.srain.cube.views.ptr.PtrFrameLayout.Mode.NONE;
            case 1:
                return in.srain.cube.views.ptr.PtrFrameLayout.Mode.REFRESH;
            case 2:
                return in.srain.cube.views.ptr.PtrFrameLayout.Mode.LOAD_MORE;
            case 3:
                return in.srain.cube.views.ptr.PtrFrameLayout.Mode.BOTH;
            default:
                return in.srain.cube.views.ptr.PtrFrameLayout.Mode.BOTH;
        }
    }
    ...
    public void refreshFinishState() {
        if (refreshStateListener != null) {
            ...
            Observable.timer(200, TimeUnit.MILLISECONDS).observeOn(AndroidSchedulers.mainThread())
                    .subscribe(aLong -> {
                        refreshStateListener.onRefreshStop();
                    });
        }
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                if (lastY - fistY < 0 && mHeaderView.getTop() == (-mHeaderView.getMeasuredHeight())
                        && !reFreshStartOnce) {
                    reFreshStartOnce = true;
                    if (refreshStateListener != null) {
                        refreshStateListener.onRefreshStop();
                    }
                }
    ...
    boolean reFreshStartOnce = true;
    boolean reFreshStopOnce = true;

    private void moveHeaderPos(float deltaY) {

        if (mHeaderView.getTop() > (-mHeaderView.getMeasuredHeight() + 10) && reFreshStartOnce) {
            reFreshStartOnce = false;
            reFreshStopOnce = true;
            if (refreshStateListener != null) {
                refreshStateListener.onRefreshStart();
            }
        }

        if (mHeaderView.getTop() < (-mHeaderView.getMeasuredHeight() + 10) && reFreshStopOnce) {
            reFreshStartOnce = true;
            reFreshStopOnce = false;
            if (refreshStateListener != null) {
                if (!isRefreshed) {
                    refreshStateListener.onRefreshStop();
                }
    ...
    public void setMode(in.srain.cube.views.ptr.PtrFrameLayout.Mode mode) {
        mMode = mode;
    }

    public in.srain.cube.views.ptr.PtrFrameLayout.Mode getMode() {
        return mMode;
    }
    ...
    RefreshStateListener refreshStateListener;
